...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    if is_user_admin(chat, user.id):
        message.reply_text("I'm not going to warn a pro shinobi!")
        return ""
...
def button(bot: Bot, update: Update) -> str:
    ...
    if match:
        ...
        if not is_user_admin(chat, int(user.id)):
            query.answer(text="Only pro shinobi or my sensei can use this command", show_alert=True)
            return ""
        res = sql.remove_warn(user_id, chat.id)
        if res:
            update.effective_message.edit_text(
                "Warn removed by a pro shinobi called {}.".format(mention_html(user.id, user.first_name)),
            ...
            update.effective_message.edit_text(
                "Shinobi has already has no warns.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML)
...
def warn_user(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id:
        ...
        message.reply_text("No shinobi was designated!")
    return ""
...
def reset_warns(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id:
        ...
        message.reply_text("No shinobi has been designated!")
    return ""
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            ...
            update.effective_message.reply_text(
                "User has {}/{} warnings, but no reasons for any of them. Mtsm pure shinobi".format(num_warns, limit))
    else:
        update.effective_message.reply_text("This shinobi hasn't got any warnings!")
...
def remove_warn_filter(bot: Bot, update: Update):
    ...
    for filt in chat_filters:
        if filt == to_remove:
            ...
            msg.reply_text("Yep, I'll stop warning shinobi for that.")
            raise DispatcherHandlerStop
